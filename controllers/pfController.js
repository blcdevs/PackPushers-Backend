...
pfController.createPackage = async (req, res) => {
  try {
    ...
    const basePrice = priceChart ? (priceChart.price * amount) : 0;
    
    // Calculate tax (12%)
    const tax = (basePrice * 0.12);

    // Add a $15 charge
    const additionalCharge = 15;

    // Calculate the final package total price
    const packageTotalPrice = basePrice + tax + additionalCharge;
...
pfController.setting = async (req, res) => {
  const { token , warehouseName, fullAddress, country, city, whatsapp, bankDetail, accountNo, bankName, routingNo, branchName, paypal } = req.body;
    const decodedToken = jwt.verify(token, JWT_SECRET_KEY);
      ...
      var pfset = await Pfsetting.findOne({ _pf_id: userId });
        ...
        pfset.routingNo = routingNo;
        pfset.branchName = branchName;
        pfset.paypal = paypal;
        await pfset.save();
