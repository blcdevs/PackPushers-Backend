...
userController.dashboard = async (req, res) => {
  try {
    ...
    const pendingShipments = await Pfshipment.countDocuments({
      userid: userId,
      status: "pending",
    });

    // for calculate pending shipments amount //
    const pendingShipmentsAmount = await Pfshipment.find({
      userid: userId,
      status: "pending",
    });
    ...
    const shippedShipmentsAmount = await Pfshipment.find({
      userid: userId,
      status: "shipped",
    });
    ...
    const pendingDeliveries = await Pfshipment.find({
      userid: userId,
      status: "pending",
    });
...
userController.signup = async (req, res) => {
  try {
    ...
    const referralLink = shortid.generate();

    user.referrerLink = referralLink;

    // Check if there's a referrer with the provided referral code
    if (referralCode) {
      const referrer = await User.findOne({ referralLink: referralCode });

      if (referrer) {
        user.referrer = referrer._id; // Track the referrer
        referrer.totalUsersReferred += 1; // Increment totalUsersReferred for the referrer
        await referrer.save();
      }
    }
    ...
    const verificationLink = `${BASE_URL}/users/verify-email/${verificationToken}`;
    const mailOptions = {
      from: EMAIL_USERNAME,
      to: user.email,
...
userController.createshipment = async (req, res) => {
  try {
    ...
    // Find the user who created the shipment
    ...
    if (userdata.referrer) {
      const referrer = await User.findById(userdata.referrer);
      if (referrer) {
        referrer.totalShipments += 1; // Increment totalShipments for the referrer
        const commission = 0.06 * charges; // Calculate commission (6% of the shipment cost)
        referrer.totalEarnings += commission; // Add commission to referrer's earnings
        await referrer.save();
      }
    }

    // Increment the user's totalHauls
    userdata.totalHauls += 1;
    await userdata.save();
...
userController.getAffiliateData = async (req, res) => {
  try {
    ...
    const affiliateData = {
      referrerLink: user.referrerLink,
      totalUsersReferred: user.totalUsersReferred,
      totalShipments: user.totalShipments,
      totalEarnings: user.totalEarnings,
    };

    // Log the affiliate data to the console
    console.log("Affiliate Data:", affiliateData);

    // Send the affiliate data as a JSON response
    res.json({ affiliateData });
  } catch (error) {
    console.log("Error in getAffiliateData:", error);
    res.status(500).json({ error: "Server error" });
  }
};
...
userController.requestWithdrawal = async (req, res) => {
  try {
    const { token } = req.body;
    const decodedToken = jwt.verify(token, JWT_SECRET_KEY);
    const userEmail = decodedToken.email;

    // Find the user who is requesting a withdrawal
    const user = await User.findOne({ email: userEmail });

    // Check if the user meets the withdrawal requirements
    if (user.totalHauls < 25) {
      return res.status(400).json({ error: "You need at least 25 hauls/shipments to withdraw." });
    }
    ...
    // Implement the actual withdrawal process here, such as transferring funds to the user's bank account or PayPal.

    res.json({ message: "Withdrawal request successful.", withdrawalAmount });
  } catch (error) {
